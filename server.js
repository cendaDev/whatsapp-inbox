...
app.use((req, res, next) => {
    console.log(`📥 ${req.method} ${req.url} - ${new Date().toLocaleString("es-CO", { timeZone: "America/Bogota" })}`);
    next();
});
                ...
                if (st) {
                    ...
                    if (status === "failed" && error) {
                        ...
                        if (
                            ...
                            console.log(`❌ ${to} no tiene WhatsApp (${detail || code}) - ${new Date().toLocaleString("es-CO", { timeZone: "America/Bogota" })}`);
                            ...
                            console.log(`⚠️ Error al enviar a ${to}: ${detail || code} - ${new Date().toLocaleString("es-CO", { timeZone: "America/Bogota" })}`);
                        }
                    }
...
app.listen(PORT, () => {
    console.log(`✅ Server escuchando en http://localhost:${PORT} - ${new Date().toLocaleString("es-CO", { timeZone: "America/Bogota" })}`);
});
